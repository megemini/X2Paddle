...
class BenchmarkPipeline:
    ...
    def run_benchmark(self,
        ...
        # compare pytorch_result and paddle_inference_result
        self.compare_result = "Compare Successed"
        if isinstance(pytorch_result, list) or isinstance(
                ...
                if isinstance(pytorch_result[i], torch.Tensor):
                    ...
                    if max_abs_diff >= abs_diff_threshold:
                        ...
                        relative_diff_all = max_abs_diff / np.fabs(
                            pytorch_result[i]).max()
                        relative_diff = relative_diff_all.max()
                        if relative_diff >= relative_diff_threshold:
                            self.compare_result = "Compare Failed"
        else:
            diff = results - pytorch_result
            max_abs_diff = np.fabs(diff).max()
            if max_abs_diff < abs_diff_threshold:
                self.compare_result = "Compare Successed"
            else:
                relative_diff = max_abs_diff / np.fabs(pytorch_result).max()
                if relative_diff < relative_diff_threshold:
                    self.compare_result = "Compare Successed"
                else:
                    self.compare_result = "Compare Failed"
