...
class BenchmarkPipeline:
    ...
    def run_benchmark(self,
        ...
        # compare tf_result and paddle_inference_result
        self.compare_result = "Compare Successed"
        if isinstance(tf_result, list) or isinstance(tf_result, tuple):
            for i in range(len(tf_result)):
                diff = results[i] - tf_result[i]
            ...
            if self.model_name == "MNasNet":
                diff = results[-1] - tf_result
            else:
                diff = results[0] - tf_result
            max_abs_diff = np.fabs(diff).max()
